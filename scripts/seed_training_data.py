...
def main():
    ...
    if write_mode:
        # Merge mode: preserve manually-added pairs (avf-*, pc-*) 
        # and replace auto-generated pairs (fqa-*)
        manual_pairs = []
        if os.path.exists(OUTPUT_PATH):
            with open(OUTPUT_PATH) as f:
                existing = json.load(f)
            # Keep pairs that were NOT auto-generated by this script
            auto_prefixes = ('fqa-', 'avf-')
            for p in existing.get('pairs', []):
                pid = p.get('id', '')
                # Keep structured avf pairs (not the generic avf-{slug}-001 ones)
                is_auto_avf = pid.startswith('avf-') and pid.endswith('-001')
                is_auto_fqa = pid.startswith('fqa-')
                if not is_auto_avf and not is_auto_fqa:
                    manual_pairs.append(p)
            print(f"\n  Preserved {len(manual_pairs)} manually-curated pairs")
        
        merged = all_pairs + manual_pairs
        # Deduplicate by ID
        seen = set()
        deduped = []
        for p in merged:
        ...
        print(f"✅ Written {len(deduped)} pairs to {OUTPUT_PATH}")
    else:
