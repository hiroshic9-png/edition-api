"""Real Estate service — query handler for Japan housing/property knowledge."""
from backend.api.services.kb_loader import load_domain
RE_DB, RE_KEYWORDS = load_domain("real_estate")

def search_real_estate(query: str) -> dict:
    query_lower = query.lower()
    results = []
    for topic_key, keywords in RE_KEYWORDS.items():
        if isinstance(keywords, list):
            ...
            score = 0
        if score > 0:
            topic_data = RE_DB.get(topic_key, {})
    ...
    return {"domain": "real_estate", "query": query, "results": results, "total_matches": len(results)}

def list_real_estate_topics() -> dict:
    topics = [{"key": k, "name_ja": v.get("name_ja",""), "name_en": v.get("name_en",""), "description": v.get("description","")} for k, v in RE_DB.items() if not k.startswith("_")]
    return {"domain": "real_estate", "topics": topics, "total": len(topics)}
