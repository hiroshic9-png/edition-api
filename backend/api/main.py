...
MCP_TOOLS = [
    {"name": "memory_store", "description": "Store conversation episodes to persistent memory. Supports Japanese language context including keigo levels and implicit subject detection. Set auto_extract=true to extract structured facts automatically. Usage: Use this to persist important conversations. For extracting facts without storing episodes, use memory_extract instead. Behavior: Write operation — creates new episode record. Idempotent if same content+session_id.", "inputSchema": {"type": "object", "properties": {"content": {"type": "string", "description": "Episode text content (Japanese or English)"}, "session_id": {"type": "string", "description": "Optional session identifier for grouping episodes"}, "role": {"type": "string", "description": "Speaker role: user, assistant, or system"}, "auto_extract": {"type": "boolean", "description": "If true, auto-extract facts from content (default: false)"}}, "required": ["content"]}, "annotations": WRITE},
    {"name": "memory_recall", "description": "Semantic search across stored memories. Handles ambiguous Japanese queries like '前回の会議で部長が言った件'. Returns ranked results with similarity scores. Usage: Use this to find past conversations. For listing structured facts, use memory_facts instead. Behavior: Read-only, no side effects.", "inputSchema": {"type": "object", "properties": {"query": {"type": "string", "description": "Search query in Japanese or English"}, "limit": {"type": "integer", "description": "Max results to return (default: 5, max: 20)"}}, "required": ["query"]}, "annotations": READ_ONLY},
    {"name": "memory_facts", "description": "List all structured facts stored as subject-predicate-object triples with confidence scores and expiration dates. Usage: Use this to review what the system knows as structured knowledge. For searching raw episodes, use memory_recall instead. Behavior: Read-only, no side effects.", "inputSchema": {"type": "object", "properties": {"valid_only": {"type": "boolean", "description": "If true, only return non-expired facts (default: true)"}}}, "annotations": READ_ONLY},
    {"name": "memory_context", "description": "Get aggregated session context summary including conversation topics, key entities mentioned, and interaction patterns. Usage: Use this to inject context into agent prompts for continuity. For raw episode data, use memory_recall. Behavior: Read-only, no side effects.", "inputSchema": {"type": "object", "properties": {"session_id": {"type": "string", "description": "Session ID to get context for (omit for global context)"}}}, "annotations": READ_ONLY},
    ...
    {"name": "regulation_industries", "description": "List all 10 regulated industries in the database with their Japanese names, key regulatory bodies, and number of compliance items. Usage: Use this to browse available industries before calling regulation_check with a specific industry. Behavior: Read-only, returns static list, no side effects.", "inputSchema": {"type": "object", "properties": {}}, "annotations": READ_ONLY},
    {"name": "regulation_tourist", "description": "Get tourist and visitor regulation categories covering customs, tax-free shopping, transportation rules, accommodation laws, and public behavior expectations. Usage: Use this for visitor/tourist compliance queries. For business industry regulations, use regulation_check instead. Behavior: Read-only, no side effects.", "inputSchema": {"type": "object", "properties": {}}, "annotations": READ_ONLY},
    ...
    {"name": "protocol_list", "description": "List all 6 Japanese business protocols with their Japanese names, categories, and brief descriptions. Usage: Use this to browse available protocols. For detailed step-by-step instructions on a specific protocol, use protocol_check. Behavior: Read-only, returns static list, no side effects.", "inputSchema": {"type": "object", "properties": {}}, "annotations": READ_ONLY},
    ...
    {"name": "calendar_list", "description": "List all calendar categories with seasonal business patterns and key deadline dates. Usage: Use this to browse calendar categories. For specific date/event queries, use calendar_check. Behavior: Read-only, returns static list, no side effects.", "inputSchema": {"type": "object", "properties": {}}, "annotations": READ_ONLY},
    ...
    {"name": "regional_list", "description": "List all regional categories covering major business regions and their distinctive characteristics. Usage: Use this to browse regions. For specific regional queries (e.g. Osaka business customs), use regional_check. Behavior: Read-only, returns static list, no side effects.", "inputSchema": {"type": "object", "properties": {}}, "annotations": READ_ONLY},
    ...
    {"name": "organization_list", "description": "List all organizational structure categories with descriptions of Japanese corporate customs. Usage: Use this to browse categories. For specific queries (e.g. keiretsu, payment terms), use organization_check. Behavior: Read-only, returns static list, no side effects.", "inputSchema": {"type": "object", "properties": {}}, "annotations": READ_ONLY},
    ...
    {"name": "foreign_entry_list", "description": "List all 6 foreign entry categories with step counts and estimated timelines for each process. Usage: Use this to browse entry procedures. For detailed step-by-step guidance on a specific procedure (e.g. visa, incorporation), use foreign_entry_check. Behavior: Read-only, returns static list, no side effects.", "inputSchema": {"type": "object", "properties": {}}, "annotations": READ_ONLY},
    ...
    {"name": "travel_list", "description": "List all travel topics covering transportation, accommodation, dining, and practical visitor information. Usage: Use this to browse travel topics. For specific travel queries, use travel_search. Behavior: Read-only, no side effects.", "inputSchema": {"type": "object", "properties": {}}, "annotations": READ_ONLY},
    {"name": "entertainment_search", "description": "Search Japan entertainment and pop culture: oshi-katsu fan culture etiquette, anime pilgrimage guides, live event manners and ticket acquisition, and seasonal cultural festivals. Usage: Use this for specific entertainment queries. For browsing all topics, use entertainment_list. Behavior: Read-only, no side effects.", "inputSchema": {"type": "object", "properties": {"query": {"type": "string", "description": "Search query in Japanese or English"}}, "required": ["query"]}, "annotations": READ_ONLY},
    {"name": "entertainment_list", "description": "List all entertainment topics covering fan culture, anime, live events, and seasonal events. Usage: Use this to browse topics. For specific queries, use entertainment_search. Behavior: Read-only, no side effects.", "inputSchema": {"type": "object", "properties": {}}, "annotations": READ_ONLY},
    {"name": "daily_life_search", "description": "Search daily life knowledge for living in Japan: postal and address systems, garbage sorting rules by municipality, utilities setup (electricity/gas/water/NHK), and healthcare navigation including national insurance. Usage: Use this for specific daily life queries. For browsing all topics, use daily_life_list. Behavior: Read-only, no side effects.", "inputSchema": {"type": "object", "properties": {"query": {"type": "string", "description": "Search query in Japanese or English"}}, "required": ["query"]}, "annotations": READ_ONLY},
    {"name": "daily_life_list", "description": "List all daily life topics with practical knowledge for residents in Japan. Usage: Use this to browse topics. For specific queries, use daily_life_search. Behavior: Read-only, no side effects.", "inputSchema": {"type": "object", "properties": {}}, "annotations": READ_ONLY},
    {"name": "language_search", "description": "Search Japanese language knowledge: keigo honorific system (sonkeigo/kenjougo/teineigo), counter words (josushi) for objects and people, name and address structure, and business Japanese templates for email, phone, and meetings. Usage: Use this for specific language queries. For browsing all topics, use language_list. Behavior: Read-only, no side effects.", "inputSchema": {"type": "object", "properties": {"query": {"type": "string", "description": "Search query in Japanese or English"}}, "required": ["query"]}, "annotations": READ_ONLY},
    {"name": "language_list", "description": "List all language topics covering keigo, counters, naming conventions, and business Japanese. Usage: Use this to browse topics. For specific queries, use language_search. Behavior: Read-only, no side effects.", "inputSchema": {"type": "object", "properties": {}}, "annotations": READ_ONLY},
    {"name": "food_search", "description": "Search Japanese food culture: dining etiquette (chopstick rules, itadakimasu), cuisine classification, restaurant navigation (shokkenki machines, izakaya ordering, sushi counter protocol), and dietary restriction guides for halal, vegetarian, and allergies. Usage: Use this for specific food queries. For browsing all topics, use food_list. Behavior: Read-only, no side effects.", "inputSchema": {"type": "object", "properties": {"query": {"type": "string", "description": "Search query in Japanese or English"}}, "required": ["query"]}, "annotations": READ_ONLY},
    ...
    {"name": "visa_search", "description": "Search Japanese visa and immigration knowledge: 29 status of residence categories, permanent residency requirements, points-based highly skilled professional system, business manager visa procedures, visa change/renewal, immigration bureau procedures, visa-exempt countries, and naturalization. Usage: Use this for specific visa/immigration queries. For browsing all topics, use visa_list. Behavior: Read-only, no side effects.", "inputSchema": {"type": "object", "properties": {"query": {"type": "string", "description": "Search query in Japanese or English"}}, "required": ["query"]}, "annotations": READ_ONLY},
        ...
        return JSONResponse({
            ...
            "result": {
                ...
                "serverInfo": {
                    ...
                    "version": "0.7.0",
                    "description": "Japan Operations OS for autonomous AI agents. 16 knowledge domains, 60+ REST endpoints, 33 MCP tools (all with annotations), 2 prompts, 2 resources. Quality-gated publishing with real-time telemetry."
                },
            ...
            "result": {"resources": [
                {
                    ...
                    "description": "All 16 knowledge domains with descriptions, endpoints, and 3-layer coverage status",
        ...
        try:
            async with httpx.AsyncClient(timeout=30) as client:
                ...
                endpoint_map = {
                    ...
                    "visa_search": ("POST", f"{base}/visa/search"),
...
def mcp_server_card():
    ...
    return JSONResponse(content={
        ...
        "description": "Japan Operations OS for autonomous AI agents. 16 knowledge domains, 55+ REST endpoints, 31 MCP tools (all with annotations), 2 prompts, 2 resources. Quality score: 96.0/100. Covers regulations, procedures, protocols, calendar, regional, organization, foreign entry, travel, entertainment, daily life, language, food culture, disaster & safety, and persistent memory.",
        "version": "0.7.0",
        "publisher": {
            "name": "EDITION",
            "url": "https://edition.sh"
        ...
        "tools": [
            ...
            {"name": "visa_search", "description": "Search visa & immigration (residence status, permanent residency, management visa, naturalization)"},
            {"name": "visa_list", "description": "List all visa & immigration topics"},
            {"name": "search", "description": "Cross-domain search across all 16 knowledge domains simultaneously"}
        ],
